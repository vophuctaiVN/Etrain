...
namespace aspnetcore.Services
{
    public interface IAccountsService
    {
        ...
        (ResultCode, AccountModel)
        Authenticate(
            AccountAuthenticateRequest requestBody
        );
        ...
        (ResultCode, QueryModel) Rank_Query(int top);
    }
    ...
    public class AccountsService : BaseService, IAccountsService
    {
        ...
        public (ResultCode, AccountModel)
        Authenticate(AccountAuthenticateRequest body)
        {
            AccountQueryDTO accountDTO =
                _procedureHelper
                    .GetData<AccountQueryDTO>("account_table_get_account",
                    new { Username = body.Username })
                    .FirstOrDefault();
            if (null == accountDTO) return (ResultCode.ACCOUNT_NOT_FOUND, null);
        ...
        public (ResultCode, QueryModel) Query(AccountQueryRequest filter)
        {
            ...
            List<AccountQueryDTO> accountDTOs =
                _procedureHelper
                    .GetData<AccountQueryDTO>("account_table_query", filter);
        ...
        public (ResultCode, int?) Create(AccountCreateRequest form)
        {
            ResultDTO result =
                _procedureHelper
                    .GetData<ResultDTO>("account_table_create",
                    new {
            ...
            if (0 > ID) return ((ResultCode) Math.Abs(ID), null); //result code là enum nên đếm thứ tự như array khi error 15 thì đếm từ 0-15 là ra.
        ...
        {
            QueryModel queryResult = new QueryModel();
            List<ProfileQueryDTO> infoDTOs =
                _procedureHelper
                    .GetData<ProfileQueryDTO>("user_info_query",
                    new { ID = account_ID });
        ...
        public (ResultCode, int?) Update_UserInfo(UserInfoUpdateRequest form)
        {
            ...
            if (null != form.Image)
                fileName =
                    string
                        .Format("{0}_1{1}",
                        form.IDaccount,
                        Path.GetExtension(form.Image.FileName)); // trường hợp không đổi ảnh thì là undefine
            else
                fileName =
                    string
                        .Format("{0}_1{1}",
            ...
            ResultDTO result =
                _procedureHelper
                    .GetData<ResultDTO>("user_info_update",
                    new {
            ...
            if (0 > productID) return ((ResultCode) Math.Abs(productID), null);
        ...
        public (ResultCode, int?) Update_ScoreInfo(ScoreInfoUpdateRequest form)
        {
            ResultDTO result =
                _procedureHelper
                    .GetData<ResultDTO>("user_scoreInfo_update",
                    new {
                        IDaccount = form.IDaccount,
                        Score = form.Score,
        ...
        public (ResultCode, QueryModel) Rank_Query(int top)
        {
            QueryModel queryResult = new QueryModel();
            List<ProfileQueryDTO> infoDTOs =
                _procedureHelper
                    .GetData<ProfileQueryDTO>("ranking_get", new { Top = top });
